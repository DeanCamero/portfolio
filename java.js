const roles = ["Software Developer ", "UI Designer ", "Web Developer ", "App Developer "];
...
function typeWriter() {
  ...
  if (!isDeleting && charIndex === currentRole.length) {
    isDeleting = true;
    typingInterval = setTimeout(typeWriter, 1500);
  } else if (isDeleting && charIndex === 0) {
    isDeleting = false;
    roleIndex = (roleIndex + 1) % roles.length;
    typingInterval = setTimeout(typeWriter, 500);
  } else {
    typingInterval = setTimeout(typeWriter, isDeleting ? 50 : 100);
  }
}
...
}, { threshold: 0.5 }); // Start when half visible
...
function Thunder(options) {
  ...
  this.render = function (ctx) {
    ...
    light.addColorStop(0, 'rgba(102, 21, 156, 0.6)');
...
function Text(options) {
  ...
  this.copy = (options.copy || 'My Portfolio') + ' ';
